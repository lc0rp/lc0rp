Tinkerer, exited fintech founder. Coder by night, product leader by day.

## Recent pursuits

| Repo                                                         | TL;DR                                                        |
| --- | --- |
| [KeepSydian](https://github.com/lc0rp/KeepSydian) | A two-way review & sync control center for Obsidian and Google Keep. |
| [CLI 365](https://github.com/lc0rp/cli-365) | An agent-friendly Go CLI for automating Outlook mail and Exchange calendars through Outlook Web Access). |
| [Boatload](https://github.com/lc0rp/boatload) | Self-hosted shared "Linear" for your semi-autonomous horde. Orchestrate, track, review. |
| [Birds Eye](https://github.com/lc0rp/birds-eye-) | Revive the hidden gems. Scan forgotten folders, turning them into an operational, hopefully manageable, project portfolio. |
| [Obsidian Task Roles](https://github.com/lc0rp/obsidian-task-roles) | Assignee and RACI-style role tracking for Obsidian tasks. |

## Links

* Connect on [LinkedIn](https://www.linkedin.com/in/lukekyohere/) or [X](https://x.com/lukekyohere) 
* Writings: [Medium](https://medium.com/@lukekyohere)

## Older projects

| Repo                                                         | Story                                                      |
| --- | --- |
| [Beyonic](https://github.com/beyonic) | Only the SDKs are public. I founded Beyonic, built its core stack and scaled it to acquisition. |
